...
type Decoder struct {
	...
	labelDecoderMap      map[string]DecoderFn
	typeDecoderMap       map[reflect.Type]DecoderFn
	opts                 []DecodeOption
...
func (d *Decoder) createDecodedNewValue(ctx context.Context, typ reflect.Type, node ast.Node, label string) (reflect.Value, error) {
	...
	if f, ok := d.typeDecoderMap[typ]; ok {
		v, err := f(node, typ)
	...
	if label != "" {
		if f, ok := d.labelDecoderMap[label]; ok {
			v, err := f(node, typ)
			if err == nil || err != ErrContinue {
				return v, err
			}
...
func (d *Decoder) decodeStruct(ctx context.Context, dst reflect.Value, src ast.Node) error {
	...
	for i := 0; i < structType.NumField(); i++ {
		...
		renderName := structField.RenderName
		v, exists := keyToNodeMap[renderName]
		if !exists {
			if structField.RenderNameFromTag {
				continue
			}

			for k, n := range keyToNodeMap {
				if strings.ToLower(strings.ReplaceAll(k, "-", "")) == renderName {
					renderName = k
					v = n
					exists = true
					break
		...
		delete(unknownFields, renderName)
		fieldValue := dst.FieldByName(field.Name)
		if fieldValue.Type().Kind() == reflect.Ptr && src.Type() == ast.NullType {
			// set nil value to pointer
			fieldValue.Set(reflect.Zero(fieldValue.Type()))
		...
		newFieldValue, err := d.createDecodedNewValue(ctx, fieldValue.Type(), v, structField.Label)
		if err != nil {
			if foundErr != nil {
				continue
			}
