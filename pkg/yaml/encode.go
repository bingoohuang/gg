...
import (
	...
	"github.com/bingoohuang/gg/pkg/yaml/printer"
...
const (
	KeyNamingDefault KeyNamingStrategy = iota
	KeyNamingRaw
	KeyNamingLowerCamel
)
...
func (e *Encoder) encodeValue(ctx context.Context, v reflect.Value, column int) (ast.Node, error) {
	...
	case reflect.Interface:
		return e.encodeValue(ctx, v.Elem(), column)
	case reflect.String:
		return e.encodeString(v.String(), column, nil), nil
	case reflect.Bool:
...
func (e *Encoder) encodeString(v string, column int, s *structFieldNameOption) ast.Node {
	v = e.keyNameChange(v, s)
	if e.isNeedQuoted(v) {
		v = strconv.Quote(v)
	}
...
func (e *Encoder) encodeMapItem(ctx context.Context, item MapItem, column int) (*ast.MappingValueNode, error) {
	...
	return ast.MappingValue(
		...
		e.encodeString(k.Interface().(string), column, nil),
		value,
	), nil
}
...
func (e *Encoder) encodeMap(ctx context.Context, value reflect.Value, column int) ast.Node {
	...
	for _, key := range keys {
		...
		node.Values = append(node.Values, ast.MappingValue(
			nil,
			e.encodeString(fmt.Sprint(key), column, nil),
			value,
		))
	}
	return node
...
func (e *Encoder) encodeStruct(ctx context.Context, value reflect.Value, column int) (ast.Node, error) {
	...
	for i := 0; i < value.NumField(); i++ {
		...
		key := e.encodeString(structField.RenderName, column, &structFieldNameOption{
			FieldName:         structField.FieldName,
			RenderNameFromTag: structField.RenderNameFromTag,
		})
		switch {
...
type structFieldNameOption struct {
	FieldName         string
	RenderNameFromTag bool
}

func (e *Encoder) keyNameChange(name string, s *structFieldNameOption) string {
	if s == nil || s.RenderNameFromTag {
		return name
	}
	switch e.KeyNaming {
	case KeyNamingDefault:
		return name
	case KeyNamingRaw:
		return s.FieldName
	case KeyNamingLowerCamel:
		return strings.ToLower(s.FieldName[:1]) + s.FieldName[1:]
	default:
		return name
	}
}
