...
func structField(field reflect.StructField) *StructField {
	...
	fieldNameFromTag := false
	options := strings.Split(tag, ",")
	if len(options) > 0 {
		if options[0] != "" {
			fieldName = options[0]
			fieldNameFromTag = true
		}
	}
	...
	if len(options) > 1 {
		for _, opt := range options[1:] {
			...
			case strings.HasPrefix(opt, "label"):
				if label := strings.Split(opt, "="); len(label) > 1 {
					structField.Label = label[1]
				}
			default:
			}
		}
