...
func NewTimingWheel(interval time.Duration, numSlots int, execute Execute) (*TimingWheel, error) {
	...
	return newTimingWheelWithClock(interval, numSlots, execute, time.NewTicker(interval))
}
...
func Recover(cleanups ...func()) {
	...
	if p := recover(); p != nil {
		log.Printf("Panic recovered from %+v, stack: %s", p, debug.Stack())
	}
}
