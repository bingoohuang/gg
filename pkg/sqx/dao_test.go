...
func openDB(t *testing.T) (*sql.DB, *sqx.Sqx) {
	// 创建数据库连接池
	rawDb, db, err := sqx.Open("sqlite3", ":memory:")
	assert.Nil(t, err)
...
func TestDaoWithError(t *testing.T) {
	...
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithError(&err)))
...
func TestDaoWithContext(t *testing.T) {
	...
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithCtx(ctx), sqx.WithLimit(1)))
...
func TestDaoWithRowScanInterceptor(t *testing.T) {
	...
	dao := &personDao2{}
	p := person{}
	f := sqx.RowScanInterceptorFn(func(rowIndex int, v ...interface{}) (bool, error) {
		p = v[0].(person)
		return false, nil
	})
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithRowScanInterceptor(f)))
...
func TestDaoWithDotSQLString(t *testing.T) {
	...
	// 生成DAO，自动创建dao结构体中的函数字段
	dao := &personDao3{Logger: &sqx.DaoLog{}}
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithSQLStr(dotSQL)))
...
func TestDaoWithDotSQLFile(t *testing.T) {
	...
	dao := &personDao3{}
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithSQLFile(`testdata/d3.sql`)))
...
func TestMapArg(t *testing.T) {
	...
	// 生成DAO，自动创建dao结构体中的函数字段
	dao := &personDaoMap{Logger: &sqx.DaoLog{}}
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithSQLStr(dotSQLMap)))
...
func TestMapDynamic(t *testing.T) {
	...
	// 生成DAO，自动创建dao结构体中的函数字段
	dao := &personDaoDynamic{Logger: &sqx.DaoLog{}}
	sqx.DB, _ = openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithSQLStr(dotSQLDynamic)))
...
func TestLastInsertID(t *testing.T) {
	...
	// 生成DAO，自动创建dao结构体中的函数字段
	dao := &personLastInsertID{Logger: &sqx.DaoLog{}}
	db, _ := openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithDB(db), sqx.WithSQLStr(dotSQLLastInsertID)))
...
type personTimeDao struct {
	...
	Query2 func(queryCond2) ([]personTime, sqx.Count, error) `sql:"select id, age, addr, birthday from person"`
...
func TestTime(t *testing.T) {
	...
	// 生成DAO，自动创建dao结构体中的函数字段
	dao := &personTimeDao{Logger: &sqx.DaoLog{}}
	db, _ := openDB(t)
	that.Nil(sqx.CreateDao(dao, sqx.WithDB(db), sqx.WithSQLStr(dotSQLTime)))
	...
	var count sqx.Count
	persons, count, err = dao.Query2(queryCond2{Addr: "%a%", Limit: sqx.Limit{Length: 10}})
	that.Nil(err)
	that.Equal([]personTime{p}, persons)
	that.Equal(sqx.Count(1), count)
