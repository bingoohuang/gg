...
func CreateDao(dao interface{}, createDaoOpts ...CreateDaoOpter) error {
	...
	for i := 0; i < structValue.NumField; i++ {
		...
		tags, err := reflector.ParseTags(string(f.Tag))
		if err != nil {
			return err
		}
...
func (option *CreateDaoOpt) getSQLStmt(field StructField, tags reflector.Tags, stack int) (SQLPart, string) {
	if stack > 10 {
		return nil, ""
	}
...
func (r *sqlRun) createFn(f StructField) error {
	...
	lastOutError := numOut > 0 && reflector.IsError(f.Type.Out(numOut-1))
	if lastOutError {
		numOut--
	}
	...
	f.Field.Set(reflect.MakeFunc(f.Type, func(args []reflect.Value) []reflect.Value {
		...
		if lastOutError {
			if err != nil {
				...
				values = append(values, reflect.Zero(reflector.ErrType))
			}
		}
...
func (p *SQLParsed) createMapFields(columns []string, out0Type reflect.Type,
	...
	mapFields := make([]selectItem, mathx.Max(lenCol, len(outTypes)))
	for i := range columns {
		if i < len(outTypes) {
			vType := out0Type
			if i > 0 {
...
func convertExecResult(result sql.Result, query string, outTypes []reflect.Type) ([]reflect.Value, error) {
	...
	firstWord := strings.ToUpper(ss.FirstWord(query))
