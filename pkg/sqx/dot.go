...
type DotScanner struct {
	line    string
	queries map[string]DotItem
	current DotItem
}

func (s *DotScanner) createNewItem(name string, tag map[string]string) {
	s.current = DotItem{Name: name, Attrs: tag, Content: make([]string, 0)}
}

type stateFn func() stateFn

func (s *DotScanner) initialState() stateFn {
...
func (s *DotScanner) queryState() stateFn {
	if tag, name := ParseDotTag(s.line, "--", "name"); name != "" {
		s.createNewItem(name, tag)
	} else {
		s.appendQueryLine()
...
func (s *DotScanner) appendQueryLine() {
...
func (s *DotScanner) Run(io *bufio.Scanner) map[string]DotItem {
	s.queries = make(map[string]DotItem)
...
func DotSQLLoad(r io.Reader) (*DotSQL, error) {
	return &DotSQL{(&DotScanner{}).Run(bufio.NewScanner(r))}, nil
}
...
func (p *PostProcessingSQLPart) Eval(env map[string]interface{}) (string, error) {
	...
	delimiter := mapp.GetStringOr(p.Attrs, "delimiter", ";")
...
// and merge to uncomment lines together.
