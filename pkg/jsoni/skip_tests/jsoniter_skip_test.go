...
func Test_skip_number_in_array(t *testing.T) {
	...
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[-0.12, "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
...
func Test_skip_string_in_array(t *testing.T) {
	should := require.New(t)
	iter := jsoni.ParseString(jsoni.ConfigDefault, `["hello", "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	should.Nil(iter.Error)
...
func Test_skip_null(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[null , "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
...
func Test_skip_true(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[true , "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	if iter.ReadString() != "stream" {
...
func Test_skip_false(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[false , "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	if iter.ReadString() != "stream" {
...
func Test_skip_array(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[[1, [2, [3], 4]], "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	if iter.ReadString() != "stream" {
...
func Test_skip_empty_array(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[ [ ], "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	if iter.ReadString() != "stream" {
...
func Test_skip_nested(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[ {"a" : [{"stream": "c"}], "d": 102 }, "stream"]`)
	iter.ReadArray()
	iter.Skip()
	iter.ReadArray()
	if iter.ReadString() != "stream" {
...
func Test_skip_and_return_bytes_with_reader(t *testing.T) {
	should := require.New(t)
	iter := jsoni.Parse(jsoni.ConfigDefault, bytes.NewBufferString(`[ {"a" : [{"stream": "c"}], "d": 102 }, "stream"]`), 4)
	iter.ReadArray()
	skipped := iter.SkipAndReturnBytes()
	should.Equal(`{"a" : [{"stream": "c"}], "d": 102 }`, string(skipped))
}
...
func Test_skip_empty(t *testing.T) {
	should := require.New(t)
	should.NotNil(jsoni.Get([]byte("")).LastError())
}
	...
	input := []byte(`
{
	...
	for n := 0; n < b.N; n++ {
		result := TestResp{}
		iter := jsoni.ParseBytes(jsoni.ConfigDefault, input)
