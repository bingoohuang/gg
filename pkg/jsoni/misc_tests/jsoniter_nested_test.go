		...
		t.Run(tc.name, func(t *testing.T) {
			for _, target := range targets {
				t.Run(target.name, func(t *testing.T) {
					err := jsoni.Unmarshal(tc.data, target.new())
...
func Test_nested(t *testing.T) {
	iter := jsoni.ParseString(jsoni.ConfigDefault, `{"hello": [{"world": "value1"}, {"world": "value2"}]}`)
	l1 := Level1{}
...
func readLevel1Hello(iter *jsoni.Iterator) []Level2 {
