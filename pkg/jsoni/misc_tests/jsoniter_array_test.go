...
func Test_empty_array(t *testing.T) {
	...
	should.False(cont)
	iter = jsoni.ParseString(jsoni.ConfigDefault, `[]`)
	iter.ReadArrayCB(func(iter *jsoni.Iterator) bool {
...
func Test_one_element(t *testing.T) {
	should := require.New(t)
	iter := jsoni.ParseString(jsoni.ConfigDefault, `[1]`)
...
func Test_encode_byte_array(t *testing.T) {
	...
	bytes, err = jsoni.Marshal([]byte{1, 2, 3})
	should.Nil(err)
	should.Equal(`"AQID"`, string(bytes))
}
...
func Test_encode_nil_byte_array(t *testing.T) {
	...
	bytes, err = jsoni.Marshal(nilSlice)
	should.Nil(err)
	should.Equal(`null`, string(bytes))
}
...
func Test_decode_byte_array_from_base64(t *testing.T) {
	...
	err = jsoni.Unmarshal([]byte(`"AQID"`), &data)
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
}
...
func Test_decode_byte_array_from_array(t *testing.T) {
	...
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
	err = jsoni.Unmarshal([]byte(`[1,2,3]`), &data)
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
}
...
func Test_decode_slice(t *testing.T) {
	...
	jsoni.UnmarshalFromString(`["hello", "world"]`, &slice)
...
func Test_decode_large_slice(t *testing.T) {
	should := require.New(t)
	slice := make([]int, 0, 1)
	jsoni.UnmarshalFromString(`[1,2,3,4,5,6,7,8,9]`, &slice)
