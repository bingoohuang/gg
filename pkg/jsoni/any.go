...
var anyType = PtrElem((*Any)(nil))

func createDecoderOfAny(_ *ctx, typ reflect2.Type) ValDecoder {
	if typ == anyType {
...
func createEncoderOfAny(_ *ctx, typ reflect2.Type) ValEncoder {
	if typ == anyType {
		return &directAnyCodec{}
	}
	if typ.Implements(anyType) {
