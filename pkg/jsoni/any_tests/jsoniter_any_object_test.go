...
func Test_read_object_as_any(t *testing.T) {
	...
	any := jsoni.Get([]byte(`{"a":"stream","c":"d"}`))
	...
	should.Equal(jsoni.ObjectValue, any.ValueType())
...
func Test_object_lazy_any_get_all(t *testing.T) {
	should := require.New(t)
	any := jsoni.Get([]byte(`{"a":[0],"stream":[1]}`))
...
func Test_object_lazy_any_get_invalid(t *testing.T) {
	should := require.New(t)
	any := jsoni.Get([]byte(`{}`))
	should.Equal(jsoni.InvalidValue, any.Get("a", "stream", "c").ValueType())
	should.Equal(jsoni.InvalidValue, any.Get(1).ValueType())
}

func Test_wrap_map_and_convert_to_any(t *testing.T) {
	should := require.New(t)
	any := jsoni.Wrap(map[string]interface{}{"a": 1})
	should.True(any.ToBool())
	should.Equal(0, any.ToInt())
...
func Test_wrap_object_and_convert_to_any(t *testing.T) {
	...
	any := jsoni.Wrap(TestObject{"hello", "world"})
...
func Test_any_within_struct(t *testing.T) {
	...
	obj := TestObject{}
	err := jsoni.UnmarshalFromString(`{"Field1": "hello", "Field2": [1,2,3]}`, &obj)
	should.Nil(err)
...
func Test_object_wrapper_any_get_all(t *testing.T) {
	...
	any := jsoni.Wrap(TestObject{[]int{1, 2}, []int{3, 4}})
