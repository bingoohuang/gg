...
func TestVeryBigBytes(t *testing.T) {
	...
	s := BigBytes(b)
	if s != "15.3ZB" {
		t.Errorf("Expected 15.3ZB, got %v", s)
	}
	s = BigIBytes(b)
	if s != "13ZiB" {
	...
	s = BigBytes(b)
	if s != "15.7YB" {
		t.Errorf("Expected 15.7YB, got %v", s)
	}
	s = BigIBytes(b)
	if s != "13YiB" {
...
func TestVeryVeryBigBytes(t *testing.T) {
	...
	s := BigBytes(b)
	if s != "16093.2YB" {
		t.Errorf("Expected 16093.2YB, got %v", s)
	}
	s = BigIBytes(b)
	if s != "13312YiB" {
