...
func ParseJSONPathExpr(pathExpr string) (pe PathExpression, err error) {
	...
	for _, indice := range indices {
		...
		if pathExprSuffix[start] == '[' {
			...
			leg := strings.TrimFunc(pathExprSuffix[start+1:end], isBlank)
			indexStr := strings.TrimFunc(leg[0:len(leg)-1], isBlank)
			var index int
			...
			key := strings.TrimFunc(pathExprSuffix[start+1:end], isBlank)
			if len(key) == 1 && key[0] == '*' {
				pe.flags |= pathExpressionContainsAsterisk
			} else if key[0] == '"' {
