...
func TestBackoffSuccess(t *testing.T) {
	...
	assert.Equal(t, externalCounter, attempts)
}
...
func TestConditionalBackoffCondition(t *testing.T) {
	...
	assert.Nil(t, retryErr)
	assert.Equal(t, counter, attempts)
}
...
func TestConditionalBackoffConditionNonExecution(t *testing.T) {
	...
	assert.Nil(t, retryErr)
	assert.NotEqual(t, counter, attempts)
}
...
func TestConditionalGet(t *testing.T) {
	...
	assert.Equal(t, externalCounter, attemptCount)
...
func TestClientRetryGet(t *testing.T) {
	...
	assert.Equal(t, "", resp.Proto())
	assert.Equal(t, 0, resp.StatusCode())
	assert.Equal(t, 0, len(resp.Cookies()))
	assert.NotNil(t, resp.Body())
	assert.Equal(t, 0, len(resp.Header()))

	assert.Equal(t, true, strings.HasPrefix(err.Error(), "Get "+ts.URL+"/set-retrycount-test") ||
...
func TestClientRetryWait(t *testing.T) {
	...
	assert.Equal(t, attempt, 6)

	// Initial attempt has 0 time slept since last request
	assert.Equal(t, retryIntervals[0], uint64(0))
...
func TestClientRetryWaitCallbackSwitchToDefault(t *testing.T) {
	...
	// 6 attempts were made
	assert.Equal(t, attempt, 6)
	assert.Equal(t, resp.Request.Attempt, 6)
	assert.Equal(t, resp.Request.TraceInfo().RequestAttempt, 6)
...
func TestClientRetryPost(t *testing.T) {
	...
	c := dc()
	c.SetRetryCount(3)
	c.AddRetryCondition(func(r *Response, _ error) bool {
...
func TestClientRetryErrorRecover(t *testing.T) {
	...
	assert.Equal(t, http.StatusOK, resp.StatusCode())
	assert.Equal(t, "hello", authSuccess.Message)
