...
func TestClientRedirectPolicy(t *testing.T) {
	...
	assert.Equal(t, true, "Get /redirect-21: stopped after 20 redirects" == err.Error() ||
		"Get \"/redirect-21\": stopped after 20 redirects" == err.Error())

	c.SetRedirectPolicy(NoRedirectPolicy())
	_, err = c.R().Get(ts.URL + "/redirect-1")
	assert.Equal(t, true, "Get /redirect-2: auto redirect is disabled" == err.Error() ||
...
func TestClientTimeout(t *testing.T) {
	...
	assert.Equal(t, true, strings.Contains(strings.ToLower(err.Error()), "timeout"))
}
...
func TestClientSetHeaderVerbatim(t *testing.T) {
	...
	assert.Equal(t, "value_lowercase", strings.Join(c.Header["header-lowercase"], "")) //nolint
	assert.Equal(t, "value_standard", c.Header.Get("Header-Lowercase"))
}
...
func TestClientSetTransport(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, true, transport == transportInUse)
}
...
func TestClientSetScheme(t *testing.T) {
	...
	assert.Equal(t, true, client.scheme == "http")
}
...
func TestClientSetCookieJar(t *testing.T) {
	...
	assert.Equal(t, true, client.httpClient.Jar == backupJar)
}

func TestClientOptions(t *testing.T) {
	client := dc()
	client.SetContentLength(true)
	assert.Equal(t, client.setContentLength, true)

	client.SetBaseURL("http://httpbin.org")
	assert.Equal(t, "http://httpbin.org", client.BaseURL)
	...
	assert.Equal(t, "application/json; charset=utf-8", client.Header.Get(hdrContentTypeKey))

	client.SetCookies(&http.Cookie{
		Name:  "default-cookie",
		Value: "This is cookie default-cookie value",
	})
	assert.Equal(t, "default-cookie", client.Cookies[0].Name)
	...
	assert.Equal(t, "default-cookie-1", client.Cookies[1].Name)
	assert.Equal(t, "default-cookie-2", client.Cookies[2].Name)
	...
	assert.Equal(t, "Param_3", client.QueryParam.Get("test_param_3"))

	rTime := strconv.FormatInt(time.Now().UnixNano(), 10)
	client.SetFormData(map[string]string{"r_time": rTime})
	assert.Equal(t, rTime, client.FormData.Get("r_time"))

	client.SetBasicAuth("myuser", "mypass")
	assert.Equal(t, "myuser", client.UserInfo.Username)

	client.SetAuthToken("AC75BD37F019E08FBC594900518B4F7E")
	assert.Equal(t, "AC75BD37F019E08FBC594900518B4F7E", client.Token)

	client.SetDisableWarn(true)
	assert.Equal(t, client.DisableWarn, true)
	...
	assert.Equal(t, rwt, client.RetryWaitTime)
	...
	assert.Equal(t, client.Debug, true)

	var sl int64 = 1000000
	client.SetDebugBodyLimit(sl)
	assert.Equal(t, client.debugBodySizeLimit, sl)

	client.SetAllowGetMethodPayload(true)
	assert.Equal(t, client.AllowGetMethodPayload, true)

	client.SetScheme("http")
	assert.Equal(t, client.scheme, "http")

	client.SetCloseConnection(true)
	assert.Equal(t, client.closeConnection, true)
}
...
func TestClientPreRequestHook(t *testing.T) {
	...
	resp, _ := client.R().
		...
		Post(ts.URL + "/login")
	assert.Equal(t, http.StatusOK, resp.StatusCode())
	assert.Equal(t, `{ "id": "success", "message": "login successful" }`, resp.String())
...
func TestClientRoundTripper(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.Nil(t, ct)
	assert.Equal(t, "current transport is not an *http.Transport instance", err.Error())
...
func TestAutoGzip(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		assert.Equal(t, "200 OK", resp.Status())
		assert.NotNil(t, resp.Body())
		assert.Equal(t, tc.want, resp.String())
...
func TestLogCallbacks(t *testing.T) {
	...
	logInfo := lgr.String()
	assert.Equal(t, true, strings.Contains(logInfo, "Bearer *******************************"))
	assert.Equal(t, true, strings.Contains(logInfo, "X-Debug-Resposne-Log"))
	assert.Equal(t, true, strings.Contains(logInfo, "Modified the response body content"))
	...
	resp, err = c.R().
		SetAuthToken("004DDB79-6801-4587-B976-F093E6AC44FF-Request").
		Get(ts.URL + "/profile")
	assert.Equal(t, errors.New("request test error"), err)
	assert.Nil(t, resp)
	assert.NotNil(t, err)
	...
	resp, err = c.R().
		SetAuthToken("004DDB79-6801-4587-B976-F093E6AC44FF-Request").
		Get(ts.URL + "/profile")
	assert.Equal(t, errors.New("response test error"), err)
	assert.NotNil(t, resp)
}
...
func TestNewWithLocalAddr(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, resp.String(), "TestGet: text response")
}
...
func TestClientOnResponseError(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.isError {
				assert.NotNil(t, err)
				assert.Equal(t, 1, hook1)
				assert.Equal(t, 1, hook2)
			} else {
				assert.Nil(t, err)
			}
		})
