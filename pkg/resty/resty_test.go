	...
	ts := createTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPost {
			...
			if r.URL.Path == "/profile" {
				...
				assert.Equal(t, true, strings.Contains(formEncodedData, "search_criteria=pencil"))
				assert.Equal(t, true, strings.Contains(formEncodedData, "search_criteria=glass"))
...
func createGenServer(t *testing.T) *httptest.Server {
	ts := createTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodOptions && r.URL.Path == "/options" {
		...
		if r.Method == http.MethodPatch && r.URL.Path == "/patch" {
			w.WriteHeader(http.StatusOK)
			return
		}
