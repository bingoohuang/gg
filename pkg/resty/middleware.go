...
func addCredentials(c *Client, r *Request) error {
	...
	isBasicAuth := userInfo != nil
	if isBasicAuth {
		r.RawRequest.SetBasicAuth(userInfo.Username, userInfo.Password)
	}
	...
	if !IsStringEmpty(token) {
		r.RawRequest.Header.Set(c.HeaderAuthorizationKey, authScheme+" "+token)
	}
...
func handleRequestBody(c *Client, r *Request) (err error) {
	...
	if reader, ok := r.Body.(io.Reader); ok {
		...
		bodyBytes = []byte(s)
	} else if IsJSONType(contentType) && (kind == reflect.Struct || kind == reflect.Map || kind == reflect.Slice) {
