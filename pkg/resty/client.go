...
// User type is to hold a username and password information
type User struct {
...
func (c *Client) SetHeader(header, value string, headerValues ...string) *Client {
	c.Header.Set(header, value)
	for i := 0; i+1 < len(headerValues); i++ {
		c.Header.Set(headerValues[i], headerValues[i+1])
	}
	return c
}
...
//		SetQueryParam("search", "kitchen papers"， "size", "large")
func (c *Client) SetQueryParam(param, value string, paramValues ...string) *Client {
	c.QueryParam.Set(param, value)

	for i := 0; i+1 < len(paramValues); i++ {
		c.QueryParam.Set(paramValues[i], paramValues[i+1])
	}
	return c
}
...
// NewRequest is an alias for method `R()`. Creates a new request instance, It's used for
...
func (c *Client) SetPreRequestHook(h PreRequestHook) *Client {
	if c.preReqHook != nil {
		c.log.Warnf("Overwriting an existing pre-request hook: %s", funcName(h))
	}
	c.preReqHook = h
	return c
}
...
func (c *Client) OnRequestLog(rl RequestLogCallback) *Client {
	if c.requestLog != nil {
		c.log.Warnf("Overwriting on-request-log callback from=%s to=%s", funcName(c.requestLog), funcName(rl))
	}
	c.requestLog = rl
	return c
}
...
func (c *Client) OnResponseLog(rl ResponseLogCallback) *Client {
	if c.responseLog != nil {
		c.log.Warnf("Overwriting on-response-log callback from=%s to=%s", funcName(c.responseLog), funcName(rl))
	}
	c.responseLog = rl
	return c
}
...
// redirect policies. Want to create one for yourself refer to `redirect.go`.
//
...
func (c *Client) SetRedirectPolicy(policies ...interface{}) *Client {
	for _, p := range policies {
		if _, ok := p.(RedirectPolicy); !ok {
			c.log.Errorf("%v should implement resty.RedirectPolicy", funcName(p))
		}
	}
...
// take advantage of some Resty client settings.
//
...
// SetTrace method enables or disables the Resty client trace for the requests fired from
...
//
// Also `Request.SetTrace` available too to get trace info for single request.
func (c *Client) SetTrace(trace bool) *Client {
	c.trace = trace
	return c
}
