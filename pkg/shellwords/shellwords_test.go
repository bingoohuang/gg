...
func TestParseMySQLCli(t *testing.T) {
	p := NewParser()
	p.ParseEnv = true
	s := `--MYSQL_PWD="! 8BE4" --mysql --host="127.0.0.1" -P 9633 -u 'root'`
	args, err := p.Parse(s)

	assert.Nil(t, err)
	assert.Equal(t, []string{"--MYSQL_PWD=! 8BE4", "--mysql", "--host=127.0.0.1", "-P", "9633", "-u", "root"}, args)
...
func TestShellRun(t *testing.T) {
	...
	pwd2, err := shellRun("pwd", path.Join(dir, "/testdata"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBacktick(t *testing.T) {
	...
	args, _ = parser.Parse(`echo $(echo "foo")`)
