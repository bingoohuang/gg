...
// #70
func TestNextSquashmapstruct(t *testing.T) {
	data := &struct {
		Level1 struct {
			Level2 struct {
				Foo string
...
// GH-140 Type error when using Hook to decode into interface
func TestDecode_DecodeHookInterface(t *testing.T) {
...

	{
		...
		decoder, _ := NewDecoder(&Config{
			Hook:   stringToPointerInterfaceDecodeHook,
			Result: decodeInto,
		})
...

	{
		...
		decoder, _ := NewDecoder(&Config{
			Hook:   stringToValueInterfaceDecodeHook,
			Result: decodeInto,
		})
