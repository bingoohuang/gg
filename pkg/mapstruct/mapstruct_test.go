...
type BasicPointer struct {
	...
	Vfloat      *float64
	Vextra      *string
	vsilent     *bool // nolint:structcheck,unused
	Vdata       *interface{}
	VjsonInt    *int
	VjsonFloat  *float64
	VjsonNumber *json.Number
...
type EmbeddedSlice struct {
	SliceAlias `mapstruct:"slice_alias"`
	Vunique    string
}
...
type EmbeddedArray struct {
	ArrayAlias `mapstruct:"array_alias"`
	Vunique    string
}

type SquashOnNonStructType struct {
	InvalidSquashType int `mapstruct:",squash"`
}
...
type Tagged struct {
	Extra string `mapstruct:"bar,what,what"`
	Value string `mapstruct:"foo"`
}

type Remainder struct {
	A     string
	Extra map[string]interface{} `mapstruct:",remain"`
}

type StructWithOmitEmpty struct {
	VisibleStringField string                 `mapstruct:"visible-string"`
	OmitStringField    string                 `mapstruct:"omittable-string,omitempty"`
	VisibleIntField    int                    `mapstruct:"visible-int"`
	OmitIntField       int                    `mapstruct:"omittable-int,omitempty"`
	VisibleFloatField  float64                `mapstruct:"visible-float"`
	OmitFloatField     float64                `mapstruct:"omittable-float,omitempty"`
	VisibleSliceField  []interface{}          `mapstruct:"visible-slice"`
	OmitSliceField     []interface{}          `mapstruct:"omittable-slice,omitempty"`
	VisibleMapField    map[string]interface{} `mapstruct:"visible-map"`
	OmitMapField       map[string]interface{} `mapstruct:"omittable-map,omitempty"`
	NestedField        *Nested                `mapstruct:"visible-nested"`
	OmitNestedField    *Nested                `mapstruct:"omittable-nested,omitempty"`
}
...
func TestDecodeTable(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			&struct {
				VisibleField interface{} `mapstruct:"visible"`
				OmitField    interface{} `mapstruct:"omittable,omitempty"`
			}{
				VisibleField: nil,
				OmitField:    "string",
			},
