...
func newObjMetadata(ty reflect.Type) *ObjMetadata {
	...
	if ty.Kind() == reflect.Struct {
		res.isStruct = true
	} else if ty.Kind() == reflect.Ptr && ty.Elem().Kind() == reflect.Struct {
		ty = ty.Elem()
	...
	if res.objKind != reflect.Invalid {
		...
		for _, fieldName := range allFields {
			metadata := newObjFieldMetadata(fieldName, res)
			res.fields[fieldName] = metadata
			res.fields[strings.ToLower(fieldName)] = metadata
		}
		for i := 0; i < res.objType.NumMethod(); i++ {
			method := res.objType.Method(i)
			res.methodNames = append(res.methodNames, method.Name)
			metadata := newObjMethodMetadata(method.Name, res)
			res.methods[method.Name] = metadata
			res.methods[strings.ToLower(method.Name)] = metadata
		}
	}
...
func (om *ObjMetadata) appendFields(fields []string, field reflect.StructField, listingType fieldListingType) []string {
	if k := field.Type.Kind(); listingType == fieldsAnonymous {
		if field.Anonymous {
	...
	} else if listingType == fieldsFlattenAnonymous && k == reflect.Struct && field.Anonymous {
		fields = append(fields, om.getFields(field.Type, listingType)...)
	} else {
		fields = append(fields, field.Name)
	}
...
func newObjFieldMetadata(name string, objMetadata *ObjMetadata) *ObjFieldMetadata {
...
func newObjMethodMetadata(name string, objMetadata *ObjMetadata) *ObjMethodMetadata {
...
func (o *Obj) IsValid() bool { return o.objKind != reflect.Invalid }
...
func (o *Obj) Fields() []ObjField { return o.getFields(fieldsNoFlattenAnonymous) }

// FieldsFlattened returns fields.
// Will not list Anonymous fields but it will list fields declared in those anonymous fields.
func (o Obj) FieldsFlattened() []ObjField { return o.getFields(fieldsFlattenAnonymous) }

// FieldsAll returns fields.
// List both anonymous fields and fields declared inside anonymous fields.
func (o Obj) FieldsAll() []ObjField { return o.getFields(fieldsAll) }

// FieldsAnonymous returns only anonymous fields.
func (o Obj) FieldsAnonymous() []ObjField { return o.getFields(fieldsAnonymous) }
...
func (o Obj) Type() reflect.Type { return o.objType }

// Kind returns the value's kind.
func (o Obj) Kind() reflect.Kind { return o.objKind }
...
func (of *ObjField) IsValid() bool { return of.valid && of.value.IsValid() }

// Name returns the field's name.
func (of *ObjField) Name() string { return of.name }

// Kind returns the field's kind.
func (of *ObjField) Kind() reflect.Kind { return of.fieldKind }

// Type returns the field's type.
func (of *ObjField) Type() reflect.Type { return of.fieldType }
...
func (of *ObjField) Tags() (Tags, error) {
	if err := of.assertValid(); err != nil {
		return nil, err
	}

	tag := of.structField.Tag
	return ParseTags(string(tag))
}
...
func (of *ObjField) IsAnonymous() bool {
	...
	return found && field.Anonymous
}

// IsExported returns true if the name starts with uppercase (i.e. field is public).
func (of *ObjField) IsExported() bool { return of.structField.PkgPath == "" }

// IsSettable checks if this field is settable.
func (of *ObjField) IsSettable() bool { return of.value.CanSet() }
...
func (om *ObjMethod) Name() string { return om.name }
...
// InTypes returns a slice with this method's input types.
func (om *ObjMethod) InTypes() []reflect.Type { return om.methodTypes(onlyInTypes) }

// OutTypes returns a slice with this method's output types.
func (om *ObjMethod) OutTypes() []reflect.Type { return om.methodTypes(onlyOutTypes) }

// IsValid returns this method's validity.
func (om *ObjMethod) IsValid() bool { return om.valid }
...
func (cr *CallResult) IsError() bool { return cr.Error != nil }
