...
func TestCallMethod(t *testing.T) {
	...
	sub, err := obj.Method("Subtract").Call(5, 6)
	assert.Nil(t, err)
...
func TestAllTags(t *testing.T) {
	...
	assert.Equal(t, tags["tag"].Value, "be")
	assert.Equal(t, tags["tag2"].Value, "1,2,3")
}
...
func TestExportedUnexported(t *testing.T) {
	...
	// But tags on unexported fields are still readable:
	{
		...
		assert.Equal(t, "ba", tags["bu"].Value)
	}
	{
...
func TestLenOfInvalidTypes(t *testing.T) {
	assert.Equal(t, 0, New(&TestStruct{}).Len())
	assert.Equal(t, 0, New(TestStruct{}).Len())

	{
		val, found := New(TestStruct{}).GetByIndex(20)
		assert.False(t, found)
		assert.Nil(t, val)
	}
	{
		val, found := New(TestStruct{}).GetByKey("nothing")
		assert.False(t, found)
		assert.Equal(t, nil, val)
	}
	{
		keys, err := New(TestStruct{}).Keys()
		assert.Nil(t, keys)
		assert.NotNil(t, err)
	}
}
